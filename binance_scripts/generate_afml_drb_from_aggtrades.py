...
class StreamingDrbCsvWriter:
    def __init__(self, path: Path) -> None:
        ...
        if self.tmp_path.exists():
            raise RuntimeError(f"Refusing to overwrite stale DRB temp output: {self.tmp_path}")
    ...
    *,
    ...
    for ewma_span in EWMA_SPAN_CANDIDATES:
        ...
        for iteration in range(1, SCALE_SEARCH_ITERATIONS + 1):
            ...
            gc.collect()
    if best is None:
        raise RuntimeError("DRB calibration produced no candidates")
    selected = dict(best)
...
def load_or_create_calibration_plan(symbol_arg: str) -> dict:
    ...
    if RESUME_CALIBRATION and output_path.exists():
        ...
        if mismatch_reason is None:
            print(f"{symbol} resume: using DRB calibration plan -> {output_path}")
            return payload
        print(f"{symbol} resume: ignoring stale DRB calibration plan ({mismatch_reason}) -> {output_path}")
    ...
    selected, candidates = calibrate_drb_parameters(
    ...
    print(
        f"{symbol} DRB calibration selected ewma_span={selected['ewma_span']} "
        f"scale={selected['threshold_scale']:.8f} bars={selected['bars']:,} "
        f"avg/day={selected['bars_per_day_avg']:.2f}",
    )
    print(f"{symbol} wrote DRB calibration plan -> {output_path}")
    return payload
...
def generate_drb_for_symbol(  # noqa: C901
    symbol_arg: str,
    calibration_payload: dict,
) -> None:
    started = time.perf_counter()
...
def main() -> None:
    ...
    for index, symbol in enumerate(SYMBOLS, start=1):
        ...
        generate_drb_for_symbol(symbol, calibration_payload)
